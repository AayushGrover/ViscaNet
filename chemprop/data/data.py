...
class MoleculeDatapoint:
    ...
    def __init__(self,
        ...
        if self.features_generator is not None:
            ...
            for fg in self.features_generator:
                ...
                if self.mol is not None and self.mol.GetNumHeavyAtoms() > 0:
                    self.gen_features.extend(features_generator(self.mol))

            self.gen_features = np.array(self.gen_features)
        ...
        if self.features is not None:
            ...
            if self.features_generator is not None:
                self.features = np.concatenate((self.features, self.gen_features))
...
class MoleculeDataset(Dataset):
    ...
    def rows(self) -> List[str]:
        """
        Returns the rows associated the molecules.

        :return: A list of rows
        """
        return [d.row for d in self._data]
        ...
        if self._batch_graph is None:
            ...
            for d in self._data:
                if d.smiles in SMILES_TO_GRAPH:
                    ...
                    mol_graph = MolGraph(d.mol, d.smiles)
                    if cache:
