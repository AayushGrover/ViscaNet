...
def get_data(dirpath: str,
    ...
    :param dirpath: Path to a directory containing CSV files.
    :param smiles_column: The name of the column containing SMILES strings. By default, uses the first column.
    ...
    all_feat_data = []
    if features_path is not None:
        for feat_path in sorted(glob(features_path)):
            features_data = []
            features_data.append(load_features(feat_path))  # each is num_data x num_features
            features_data = np.concatenate(features_data, axis=1)
            all_feat_data.append(features_data)
            
        all_feat_data = np.concatenate(all_feat_data, axis=0)    
        
    else:
        all_feat_data = None
    
    # print(len(all_feat_data[0]))
    
    skip_smiles = set()

    # Load data
    all_smiles, all_targets, all_rows = [], [], []
    for path in sorted(glob(dirpath)):
        with open(path) as f:
            reader = csv.DictReader(f)
    ...
    data = MoleculeDataset([
        MoleculeDatapoint(
            ...
            features=all_feat_data[i] if all_feat_data is not None else None
